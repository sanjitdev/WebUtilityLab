...
2. **The cap constant is exported from `src/lib/file-size-cap.ts` (NEW).** A single module owns the constant and the pure check function. The module exports:
   ...
   No other module may define `50 * 1024 * 1024` for the file-size cap. The single-source-of-truth is the new module. Tests pin this (`tests/dropzone-file-cap.test.ts` AC19a — the spec's earlier reference to a separate `tests/file-size-cap.test.ts` was a pre-implementation sketch; the actual implementation consolidated AC19a's runtime assertions + the AC19a-AC19c boundary tests into the single per-story file, mirroring S03.1's `tests/dropzone.test.ts` and S03.2's `tests/dropzone-drag-paste.test.ts` convention of one test file per story).
...
13. **Tests** at `tests/dropzone-file-cap.test.ts` (NEW; the consolidated file replaces an earlier-draft reference to a separate `tests/file-size-cap.test.ts` — the spec's pre-implementation sketch had two files, but the actual implementation uses one per-story file, mirroring S03.1's `tests/dropzone.test.ts` and S03.2's `tests/dropzone-drag-paste.test.ts` convention), keeping `tests/dropzone-drag-paste.test.ts` (S03.2) untouched for per-story regression tracking. The S03.2 file is patched (the AC18f regex widening) per Task 3.5 below. Mirrors the `node:fs` + `node:path` + `node:url` + `vitest` convention. Source-grep on `src/components/Dropzone.svelte`, `src/lib/file-size-cap.ts`, `src/App.svelte`. Coverage (15 AC19a-AC19o describe blocks; actual count: 65 sub-assertions — see AC16 below for the per-AC breakdown):
    - **AC19a (cap module exists, single source of truth)** — `src/lib/file-size-cap.ts` exists. Contains `export const MAX_FILE_SIZE_BYTES = 50 * 1024 * 1024` (regex `\bMAX_FILE_SIZE_BYTES\s*=\s*50\s*\*\s*1024\s*\*\s*1024\b` — exact constant definition). Exports `isWithinFileCap` AND `assertWithinFileCap` (both as named exports). The `assertWithinFileCap` return type is a discriminated union — the test pins the union shape by asserting the source contains both literal types. The regex MUST tolerate multi-line formatting (Prettier may break `{ kind: 'ok'; file: File }` across lines); the pattern uses `[^}]*` (negated char class, anchored to the literal's closing brace) between tokens: `\{\s*kind\s*:\s*['"]ok['"]\s*;[^}]*file\s*:\s*File[^}]*\}` for the ok branch and `\{\s*kind\s*:\s*['"]oversize['"]\s*;[^}]*size\s*:\s*number[^}]*cap\s*:\s*number[^}]*\}` for the oversize branch. The `[^}]*` anchor (not `[\s\S]*?`) is load-bearing — the early-draft `[\s\S]*?` form could leak past the literal's closing brace and accept a malformed union like `{ kind: 'oversize'; size: number; cap: number; file: File }` (which would violate AC19o's no-File-reference contract). The corrected form was applied at Review #1.
    - **AC19b (`MAX_FILE_SIZE_BYTES` is exactly 50 MiB)** — `tests/dropzone-file-cap.test.ts` (the same per-story file as AC19a) imports the constant and asserts `MAX_FILE_SIZE_BYTES === 50 * 1024 * 1024 === 52428800`. This is a runtime assertion that the constant resolves to the documented value (guards against accidental edits).
...
16. **`tests/dropzone-file-cap.test.ts` passes in the production gate.** The test file is committed, runs at `npm test`, and all assertions pass on first implementation. Actual test count: **65 sub-assertions across 15 AC19a-AC19o describe blocks** (corrected at Review #2: the spec's earlier "~50" estimate was off — AC19l alone contributes ~30 assertions via its 15-pattern × 2-file inline loop, and AC19c's review-#1 patch added 2 more boundary cases for `assertWithinFileCap` (size=0, 200MB)). The 65 number tracks the actual implementation; future stories can use this as the baseline.

## Verification

1. `npm test` → all tests pass (420 from before S03.3 + 65 new in `tests/dropzone-file-cap.test.ts`).
