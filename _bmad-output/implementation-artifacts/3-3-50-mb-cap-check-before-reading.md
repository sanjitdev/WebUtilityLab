...
5. **The dropzone reads `MAX_FILE_SIZE_BYTES` only from `src/lib/file-size-cap.ts`.** No inline literal `50 * 1024 * 1024` in `Dropzone.svelte`. No inline literal `52428800` (the decimal equivalent). The test pins this — `tests/dropzone-file-cap.test.ts` AC19d asserts the dropzone imports `assertWithinFileCap` from `'$lib/file-size-cap'` (or the relative `../lib/file-size-cap`) and does NOT contain any `* 1024 * 1024` literal outside the import.
6. **An under-cap file flows through unchanged.** When `file.size <= MAX_FILE_SIZE_BYTES`, `handleDrop` emits `onaccept?.({ kind: 'drop', file })` exactly as S03.2 shipped. The existing S03.2 test (`tests/dropzone-drag-paste.test.ts` AC18f — `handleDrop body invokes onaccept with { kind: "drop", file }`) **must still pass** without modification. S03.3 does not regress the under-cap path. (S03.7 will wire the App.svelte consumer; S03.3 leaves App.svelte unchanged.)
7. **An over-cap file emits the oversize signal, not the drop signal.** When `file.size > MAX_FILE_SIZE_BYTES`, `handleDrop` emits `onaccept?.({ kind: 'oversize', size: file.size, cap: MAX_FILE_SIZE_BYTES })` and does NOT emit `{ kind: 'drop', file }`. The two payloads are mutually exclusive on a single drop event. The M3 dead-code guard in the dropzone test (`tests/dropzone-drag-paste.test.ts:233-245`) still holds — the new oversize branch must NOT be wrapped in `if (false)` or `&& false`.
...
13. **Tests** at `tests/file-size-cap.test.ts` (NEW) for the new module, plus extension of `tests/dropzone-drag-paste.test.ts` to cover the dropzone-side routing (or — per S03.1/S03.2 per-story test convention — `tests/dropzone-file-cap.test.ts` NEW, keeping `tests/dropzone-drag-paste.test.ts` (S03.2) untouched for per-story regression tracking). **Spec choice: `tests/dropzone-file-cap.test.ts` NEW**, parallel to S03.1's `tests/dropzone.test.ts` and S03.2's `tests/dropzone-drag-paste.test.ts`. Mirrors the `node:fs` + `node:path` + `node:url` + `vitest` convention. Source-grep on `src/components/Dropzone.svelte`, `src/lib/file-size-cap.ts`, `src/App.svelte`. Coverage (15 AC19a-AC19o describe blocks):
    ...
    - **AC19d (dropzone imports from the cap module, not inline)** — `src/components/Dropzone.svelte` script block contains `import\s*\{[^}]*\bassertWithinFileCap\b[^}]*\}\s*from\s*['"](\$lib|\.\.\/lib)\/file-size-cap['"]` (or `'./file-size-cap'` if the module is co-located, but S03.3 places it in `src/lib/`). AND `dropzoneSource` does NOT contain `50\s*\*\s*1024\s*\*\s*1024` (the inline-literal negative pin).
    ...
    - **AC19l (no forbidden source patterns in NEW code, Privacy Baseline + AD-7)** — `dropzoneSource` AND `fileSizeCapSource` do NOT contain `\bfetch\(`, `XMLHttpRequest`, `EventSource`, `sendBeacon`, `navigator\.sendBeacon`, `new Function`, `\beval\b`, dynamic `import\(`, `\bFileReader\b`, `\breadAsText\b`, `\breadAsArrayBuffer\b`, `\breadAsBinaryString\b`, `\breadAsDataURL\b`. The new module + the new dropzone code must NOT read the file's contents (the cap is on `file.size` only; the bytes are never touched). **The `FileReader` / `readAsText` / `readAsArrayBuffer` / `readAsBinaryString` / `readAsDataURL` negatives are the load-bearing "do not read the file" pin.** A future contributor who tries to "validate" an over-cap file by peeking at the first few bytes trips here. Mirrors S03.1 AC17g, extended with the file-read negatives.
    ...
    - **AC19o (`onaccept` payload kind: 'oversize' does NOT carry a `File` reference)** — `dropzoneSource`'s `onaccept` discriminated union for the oversize branch is `{ kind: 'oversize'; size: number; cap: number }` (no `file` field). The test greps for the union type annotation and asserts no `file:` field appears inside the oversize literal type. The intent: an over-cap file's bytes are NEVER held in app memory (the `File` object would still hold a reference to the browser's file handle, which the spec excludes). Downstream consumers (S03.4 aria-live, S03.7 reducer, S03.9 strict-brief) format the error from `{ size, cap }` only.
...
16. **`tests/dropzone-file-cap.test.ts` passes in the production gate.** The test file is committed, runs at `npm test`, and all assertions pass on first implementation. Expected test count: ~20 tests across 15 AC19a-AC19o describe blocks (some ACs have multiple sub-assertions, mirroring the S03.1 / S03.2 pattern).
...
- [ ] **Task 3** — Modify `src/components/Dropzone.svelte` to add cap routing:
  - [ ] 3.1 SCRIPT BLOCK additions:
    - Add `import { assertWithinFileCap } from '$lib/file-size-cap';` (or `from '../lib/file-size-cap'`; the S03.3 spec uses `$lib/` per the project's path-alias convention, mirrors future E05 reducer imports).
    ...
    - Add `function handlePickerChange(event: Event): void` per AC9: read input.files[0], route through assertWithinFileCap, fire onaccept with the appropriate kind, reset `input.value = ''` after the consumer runs.
...
- [ ] **Task 5** — Write `tests/dropzone-file-cap.test.ts` (NEW):
  ...
  - [ ] 5.5 AC19d: dropzone imports from the cap module, no inline literal. Grep `dropzoneSource` for `import\s*\{[^}]*\bassertWithinFileCap\b[^}]*\}\s*from\s*['"](?:\$lib|\.\.\/lib)\/file-size-cap['"]`. Assert negative: `dropzoneSource` does NOT contain `50\s*\*\s*1024\s*\*\s*1024` (the inline literal would indicate cap duplication).
...
The third most likely finding: **`input.value = ''` ordering inside `handlePickerChange`.** The reset must happen AFTER `onaccept?.(...)` runs (so the consumer can still read the File reference from `input.files[0]` if it wants). The test pins positional ordering. A future contributor who moves the reset to the top of the function (to "clear early") trips here. Document the ordering rationale.
